...
```{r}
# CD4 Memory CD8 Memory_2          GMP     Prog_B 1     Prog_B 2     Prog_RBC         Treg 
...
p
# filename <- "check_B-ALL_celltype_48h.pdf"
# path <- paste0(Task3_UMAP_after_filter)
# ggsave(filename = file.path(path, filename), plot = p, width = 10, height = 8)
```
...
p
# filename <- "check_B-ALL_newlabels_48h.pdf"
# path <- paste0(Task3_UMAP_after_filter)
# ggsave(filename = file.path(path, filename), plot = p, width = 10, height = 8)
```
...
```{r}
# KO_KO_CAR_48 KO_KO_Mock_48  KO_WT_CAR_48 KO_WT_Mock_48  WT_KO_CAR_48 WT_KO_Mock_48  WT_WT_CAR_48 
#     "#BB4659"     "#A66A1B"     "#797D1D"     "#3B8B42"     "#3C9183"     "#378DB7"     "#5A52C8" 
# WT_WT_Mock_48 
#     "#B147A9" 
```
...
```{r}
# p <- SCpubr::do_DimPlot(B_ALL_CAR.final.all.rerun, 
#                         split.by = "new_labels", 
#                         plot.axes = TRUE,
#                         ncol = 4, 
...
#                         font.size = 24)
# 
# filename <- "Task3.5_Combined_UMAP_48h.pdf"
# path <- paste0(Task3.5_Individual_Sample_UMAP)
# ggsave(filename = file.path(path, filename), plot = p, width = 30, height = 20)
```
...
ko_ko_car <- subset(seurat_object_b_merged_48_72, subset = new_labels %in% c("KO_KO_CAR_48", "KO_KO_CAR_72"))
...
for (group in groups) {
  ...
  p <- UMAPPlot(subset_seurat, label = FALSE, group.by = "new_labels", pt.size = 0.2) +
    # ggtitle(paste("UMAP for", group, ": 48 hours")) +
    ggtitle(paste("UMAP for", group, ": 72 hours")) +
    theme_minimal()
...
combined_plot <- wrap_plots(plot_list, ncol = 3) 
# ggsave(paste0(Task5.3.1_48h_vs_72h, "/All_UMAP_48h.pdf"), plot = combined_plot, width = 20, height = 15)
# ggsave(paste0(Task5.3.1_48h_vs_72h, "/All_UMAP_72h.pdf"), plot = combined_plot, width = 20, height = 15)
```

```{r}
######### Merge 48h and 72h Samples, 48h vs. 72h

seurat_object_b_48 <- readRDS(paste0(save_point, "/B-ALL_clusters_final_rerun_Cluster1and2_48h.rds"))
seurat_object_b_72 <- readRDS(paste0(save_point, "/B-ALL_clusters_final_rerun_Cluster1and2_72h.rds"))
```

```{r}
# Merge the two Seurat objects
seurat_object_b_merged_48_72 <- merge(
  x = seurat_object_b_48, 
  y = seurat_object_b_72,
  merge.dr = "umap"
)
```

```{r}
table(seurat_object_b_merged_48_72@meta.data$new_labels)
```
...
for (group in groups) {
  # Extract the specific time points for each group
  time_points <- grep(group, names(table(seurat_object_b_merged_48_72@meta.data$new_labels)), value = TRUE)
  
  # Subset the Seurat object for these time points
  subset_seurat <- subset(seurat_object_b_merged_48_72, subset = new_labels %in% time_points)
...
# 6. **nono Combine with Pool7 and Pool8
